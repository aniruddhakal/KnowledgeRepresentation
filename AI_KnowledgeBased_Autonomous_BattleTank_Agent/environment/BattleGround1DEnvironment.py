...
# class BattleGround1DEnvironment(BattleGround):
class BattleGround1DEnvironment:
    """
        :author: Aniruddha Kalburgi
        """

    def __init__(self, _field_length=10):
        # super().__init__(_field_length=_field_length)
        self.fieldLength = _field_length
        self.battlefield = np.zeros((self.fieldLength,), dtype=np.uint32)
    ...
    def represent_this_world(self):
        ...
        target_line = ""
        target_index_line = ""
        for i in range(0, self.battlefield.shape[0]):
...
def main():
    bg = BattleGround1DEnvironment(_field_length=10)
    bg.initEnvironment()
